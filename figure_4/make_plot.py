...
def get_pathways(filename, taxonomy, expression):
	...
	for line in open(filename):
		...
		protein=cut[2]
		paths = cut[4]
		if "Photosynthesis" not in paths and protein!="ribulose-bisphosphate carboxylase large chain":
			continue
...
def rename_labels(labels):
	for i,label in enumerate(labels):
		...
		elif "ribulose-bisphosphate" in label:
			label="rbcL\n(RuBisCO)"
		else:
			label=label
		print label
...
ax.legend(handles[3:], ["$\it{Dolichomastix}$ (chloroplast)", "$\it{Euhalothece}$","$\it{Halothece}$"], loc='upper center', ncol=3, fontsize=12)


ax.set_ylabel("Total expression (Log10 TPM)")
ax.set_xlabel("Photosynthetic and carbon fixation genes")
