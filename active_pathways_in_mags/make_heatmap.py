...
def load_mags(filename):
	...
	for line in open(filename):
		cut=line.strip().split("\t")
		if "Micromonas" in cut[1]:
			mag = "Dolichomastix_55_7"
		elif "Halothece" in cut[1] or "Euhalothece" in cut[1]:
			mag = "_".join(cut[1].split("_")[1:])
		else:
			continue
		taxonomy[cut[0]]=mag
		if mag not in mags:
...
def get_pathway_ratios(filename, dna_df, rna_df, taxonomy, toi):
	...
	for line in open(filename):
		...
		if contig not in taxonomy:
			continue
		taxon = taxonomy[contig]
		if taxon not in toi:
			continue
		
		for path in paths:
			skip=False
			key_words=["Global", "Ribosome"]
...
def get_ratios(df):
	...
	for rna_sample in data:
		sample = "_".join(rna_sample.split("_")[:-1])
...
def drop_incomplete_paths(pathways, gene_lists):
	...
	for path in pathways:
		...
		for taxon_sample in pathways[path]:
			taxon = "_".join(taxon_sample.split("_")[:-1])
...
##################   LOADING DATA   ######################
mags,taxonomy = load_mags("contigs.mags")
