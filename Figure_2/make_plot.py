...
def draw_averages (taxon_info, ax):
	...
	for taxa in taxon_info:
		...
		ax.scatter(ratio, abund, c=color, s=total_abund/100, alpha=0.6)
		xs.append(ratio)
		ys.append(abund)
		if "_" not in taxa:
			ax.text(ratio*1.1, abund*1.02, taxa, fontsize=axis_font)
		if taxa=="T17_Micromonas_55_7":
			ax.text(ratio*1.1, abund*1.02, "Chromosomes", fontsize=axis_font)
		if taxa=="T17_Micromonas_42_105":
			ax.text(ratio*1.1, abund*1.02, "Chloroplast", fontsize=axis_font)
		if taxa=="T17_Micromonas_48_77":
			ax.text(ratio*1.1, abund*1.02, "Mitochondria", fontsize=axis_font)
...
def color_legend(ax):
	...
	legend_elements = []
	for taxon in scheme:
		legend_elements.append(Patch(facecolor=scheme[taxon], edgecolor=scheme[taxon], label=taxon, linewidth=1))
	ax.legend(handles=legend_elements, loc="lower center", framealpha=1, frameon=True, facecolor='w', ncol=2, columnspacing=1, handlelength=1, prop={'size': label_font})
