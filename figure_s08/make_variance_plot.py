...
def compute_variance(df, taxon, vars_df):
	data=df.T.to_dict()
	if taxon=="Cyanobacteria": itaxon=r"$Cyanobacteria$"
	elif taxon=="Euryarchaeota": itaxon=r"$Euryarchaeota$"
	elif taxon=="Bacteroidetes": itaxon=r"$Bacteroidetes$"
	elif taxon=="Proteobacteria": itaxon=r"$Proteobacteria$"
	elif taxon=="Actinobacteria": itaxon=r"$Actinobacteria$"
	elif taxon=="Nanoarchaeota": itaxon=r"$Nanoarchaeota$"
	else: itaxon=taxon
	...
	for path in data:
		for type in ["DNA", "RNA"]:
			...
			var = np.var(values)
			line = pd.DataFrame({"Taxon":[itaxon], "Type":[type], "Variance":var})
