...
def load_contig_bins(dirname):
	print "bin contig memberships"
	data={}
	for filename in os.listdir(dirname):
		binname="_".join(filename.split(".")[:-1])
...
samples=["9AM-1", "9AM-2", "9AM-3", "9AM-4", "9AM-5", "9AM-6", "9PM-1", "9PM-2","9PM-3","9PM-4","9PM-5","9PM-6"]
ct=0
for y in range(len(axes)):
	for x in range(len(axes[y])):
		virus_dna_vs_rna(viruses, dna_coverage, rna_coverage, hosts, samples[ct], axes[y][x])
		if x==0:
			axes[y][x].set_ylabel("Average gene transcription (TPM)")
		else:
			axes[y][x].yaxis.label.set_visible(False)
		if y==3:
			axes[y][x].set_xlabel("Contig abundance (CPM)")
		else:
			axes[y][x].xaxis.label.set_visible(False)
		ct+=1


plt.tight_layout()
plt.savefig("bubble_plot.png", dpi=600)
plt.show()
